...
static bool submit_multiple_pkts(struct xsk_socket_info *xsk, struct metadata_hdr *meta_hdr,
	...
	*/
	...
	for(int i = 0; i < num_pkts_send; i++) {
		...
		size_t data_len = 0;
		char data_buffer[array_net_meta[i]->udp.length];
		printf("%d %d\n", array_net_meta[i]->bth.psn, array_net_meta[i]->bth.opcode);
		//memcpy(data_buffer, &send_buffer[xsk->cpu_id][array_net_meta[i]->seq_num], array_net_meta[i]->data_len);
		create_packet(pkt, &data_len, p_info, NULL, data_buffer, array_net_meta[i]->udp.length);
...
static void send_packet_directly(struct xsk_socket_info *xsk) {
    ...
    create_packet(pkt, &data_len, p_info, NULL, NULL, 0);
	//printf("\n%lu", data_len);
...
void trigger_idle_flow(struct pkt_info p_info, struct xsk_socket_info *xsk) {
	...
	int ip_id = 65535;
	create_packet(data, &data_len, p_info, &ip_id, NULL, 0);
...
int set_initial_ctx_values(__u32 f_id, int context_fd) {
	...
	new_ctx.num_SQ_list = 0;
	new_ctx.num_RQ_list = 0;
	new_ctx.num_sent_packets = 0;
	new_ctx.transport_timer = 1;
	new_ctx.nPSN = 0;

	new_ctx.num_responder_packets = 0;

	new_ctx.ePSN = 0;
	new_ctx.MSN = 0;
	new_ctx.credit_count = 0;
	new_ctx.recv_first_psn = 0;
	new_ctx.write_first_psn = 0;

	new_ctx.MTU = 1000;
	new_ctx.init_sqn = 0;
	new_ctx.dest_qp = f_id;
	new_ctx.src_port = f_id;
	new_ctx.dst_port = f_id;

	new_ctx.Rt = 0;
	new_ctx.Rc = 0;
	new_ctx.alpha = 0;
	new_ctx.byte_counter = 0;
	new_ctx.BC = 0;
	new_ctx.T1 = 0;
	new_ctx.F = 0;
	new_ctx.Rai = 0;
	new_ctx.ecn_found_counter = 0;

	new_ctx.first_pkt_connection = 1;
