...
// https://github.com/xdp-project/bpf-examples/blob/master/AF_XDP-interaction/af_xdp_user.c#L1420
static void update_xskmap(int xsk_map_fd, struct xsk_socket_info *xsk_info) {
	int fd = xsk_socket__fd(xsk_info->xsk);
	int key = 2;

	int ret = bpf_map_update_elem(xsk_map_fd, &key, &fd, 0);
	if(ret) {
		fprintf(stderr, "Couldn't update XSK map: %d Key: %d XSK Socket: %d\n",
			xsk_map_fd, key, fd);
	}
}
						    ...
						    struct xsk_umem_info *umem)
{
	...
	if (custom_xsk) {
		...
		if (ret)
			goto error_exit;
		//update_xskmap(xsk_map_fd, xsk_info);
	} else {
